<template>
  <div class="app-container">
    ...
    <div style="margin-bottom: 15px">
      累计利息:<span style="color:red">{{member_interest.toFixed(2)}}</span>
      累计保证金:<span style="color:red">{{sum_depo.toFixed(2)}}</span>
      累计总资产:<span style="color:red">{{sum_cap.toFixed(2)}}</span>
      累计申请资金:<span style="color:red">{{sum_lev.toFixed(2)}}</span>
      累计盈亏:<span style="color:red">{{sum_profit.toFixed(2)}}</span>
    </div>
    <el-table
      :key="tableKey"
    ...
    >
      ...
      <el-table-column :label="$t('heyue.username')" width="60px" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.member.nickname }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('heyue.total_capital')" width="80px" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.total_capital }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('heyue.deposit')" width="80px" align="center">
        <template slot-scope="scope">
      ...
      <el-table-column :label="$t('heyue.leverage_money')" width="80px" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.leverage_money }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('heyue.loss_warning_line')" width="80px" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.loss_warning_line }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('heyue.loss_sell_line')" width="80px" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.loss_sell_line }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('heyue.interest_rate')" width="80px" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.interest_rate }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('heyue.interest')" width="80px" align="center">
        <template slot-scope="scope">
      ...
      <el-table-column :label="$t('heyue.sum_interest')" class-name="status-col" width="80px">
        <template slot-scope="scope">
          <span>{{ scope.row.member_interest }}</span>
        </template>
      </el-table-column>
      <el-table-column label="合约盈亏" class-name="status-col" width="80px">
        <template slot-scope="scope">
          <span>{{ (scope.row.market_value  - scope.row.deposit - scope.row.leverage_money).toFixed(2) }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('heyue.capital_used_time')" width="80px" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.capital_used_time }}天</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('heyue.repare_capital')" width="80px" align="center">
        <template slot-scope="scope">
      ...
      <el-table-column label="关闭时间" width="100px" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.close_date }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('heyue.apply_state')" align="center" min-width="150px" class-name="small-padding fixed-width">
        <template slot-scope="{row}">
          <el-button v-permission="['passHeYue']" v-if="row.apply_state === 0" size="mini" :type="'success'" @click="handleModifyStatus(row,1,'published')">
            通过
          </el-button>
          <el-button v-permission="['passHeYue']" v-if="row.apply_state === 0" size="mini" :type="'danger'" @click="handleModifyStatus(row,3,'published')">
            拒绝
          </el-button>
  ...
  methods: {
    ...
    getList() {
      ...
      fetchList(this.listQuery).then(response => {
        ...
        this.sum_cap = response.data.cap
        this.sum_lev = response.data.lev
        this.sum_profit = response.data.profit
        this.sum_depo = response.data.deposit
